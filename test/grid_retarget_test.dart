...
void main() {
  ...
  group('AppNotifier.retargetPayload', () {
    ...
    // `grid` and an explicit `grid: null` the same way, so "own login" has to say so in its own
    // field. This is the one constraint the whole per-agent-model wire change exists to protect, and
    // until now it was guaranteed only by reading the code.
